  ...
  .controller(
    ...
    [
      ...
      function ($scope, $routeParams, data) {
        ...
        var base_title = 'Heather Nunnelly';

        $scope.title = base_title + ''; // make new string with empty concat
        $scope.body_class = '';
        $scope.description = '';
        ...
        $scope.$on('$routeChangeSuccess', function () {
          data.success(
            function (data, status, xhr) {
              if (!$routeParams.image) {
                $scope.body_class = 'showcase';
                $scope.description = data.showcases[$routeParams.showcase].description;
                $scope.title = base_title + ' | ' + data.showcases[$routeParams.showcase].title;
              } else {
                $scope.body_class = 'image';
                $scope.description = data.showcases[$routeParams.showcase].images[$routeParams.image].description;
                $scope.title = base_title + ' | ' +
                  data.showcases[$routeParams.showcase].images[$routeParams.image].title;
              }
            }
          );
        });
